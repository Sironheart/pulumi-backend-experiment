...
(any SSO provider), service-managed AES-GCM secrets. GoReleaser publishes binary archives
and a multi-architecture container image built with ko.

## Layout

- `cmd/pulumi-backend` — entrypoint
- `internal/config` — YAML config: issuer, clientId, signingKey, secretsKey, bucket, region, noAuth
...
- `internal/secrets` — HKDF-derived, per-stack AES-GCM crypter
...
- Handler tests: in-memory fakes. AWS integration: floci via testcontainers-go
  (S3 conditional writes and STS covered); LocalStack only if floci gaps.
...
- No multi-account support: one bucket, ambient credentials.
